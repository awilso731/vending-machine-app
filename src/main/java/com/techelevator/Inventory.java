...
public class Inventory {
    ...
    public void loadFromFile(String fileName) {
        ...
        try(Scanner scanner = new Scanner(inventoryFile)) {
            ...
            while (scanner.hasNextLine()){
                ...
                //this.products.add(lineOfText);
                //System.out.println(myproduct.getSlotIdentifier());
            }
    ...
    public void setLoadedFile(Scanner insertedFile) {
        this.loadedFile = insertedFile;
        Product myproduct;
        while (loadedFile.hasNextLine()){
            String lineOfText = loadedFile.nextLine();
            String[] textParts = lineOfText.split("\\|");
            // C3|Mountain Melter|1.50|Drink, it's in 4 separate parts due to string being separated by '|'
            // C3, Mountain Melter, 1.50, Drink
            myproduct = new Product(textParts[1], textParts[3], Double.parseDouble(textParts[2]));
            this.productInventory.put(textParts[0], myproduct);
            //this.products.add(lineOfText);
            System.out.println(myproduct.getSlotIdentifier());
        }
